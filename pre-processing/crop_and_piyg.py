...
output_dir = "/content/drive/MyDrive/Faculdade/TCC/src/APTOS_processed_PiYG_256"
...
def create_piyg_colormap():
    # Gera um colormap do Matplotlib
    cmap = plt.get_cmap('PiYG')
    piyg_colors = (cmap(np.arange(cmap.N)) * 255).astype(np.uint8)
    return piyg_colors[:, :3]  # Retire o canal alpha

def apply_custom_colormap(img, colormap):
    # Mapeia os valores de intensidade para o colormap
    return colormap[img]

def apply_clahe_piyg(img):
    ...
    colormap = create_piyg_colormap()
    colored_img = apply_custom_colormap(clahe_img, colormap)
    return colored_img
